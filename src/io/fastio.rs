...
impl Fastio {
    ...
    ///
    /// # Complexity
    /// - 時間計算量: $O(N)$、ここで $N$ は出力済みバイト数 (未フラッシュ分のフラッシュ処理を含む)。
    /// - 空間計算量: $O(1)$ (所有権の移動のみ)
    ///
    /// # Examples
    /// ```rust
    ...
    ///
    /// # Complexity
    /// - 時間計算量: 償却 $O(d)$、ここで $d$ は `value` を書き込むために必要なバイト数。
    /// - 空間計算量: $O(1)$
    ///
    /// # Examples
    /// ```rust
