...
impl<W: Copy> BellmanFord<W> {
    ...
    ///
    /// # Returns
    /// `Option<W>`: `v` までの最短コスト。いずれの始点からも到達できない場合、
    /// または `v` が負閉路の影響を受ける場合は `None`
    ...
    ///
    /// # Returns
    /// `Option<Vec<usize>>`: 始点から `v` に至る頂点列。`v` にいずれの始点からも
    /// 到達できない場合、または `v` が負閉路の影響を受ける場合は `None`。
    ///
    /// # Complexity
    /// - 時間計算量: O(経路長)
...
impl<T: Copy + Ord + std::ops::Add<Output = T>> graph::Graph<T> {
    ...
    ///
    /// # Args
    /// - `starts`: `(始点, 初期コスト)` の組の列。同じ頂点が複数回現れた場合、
    ///   最も小さい初期コストが採用される。
    ///
    /// # Returns
