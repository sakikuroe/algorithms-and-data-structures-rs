...
impl<T> graph::Graph<T> {
    ...
    /// 並べた列。頂点列の長さは辺のペイロード列より常に1大きい (`vertices[i]`
    /// から `payloads[i]` を通って `vertices[i+1]` へ至る)。多重辺があっても
    /// どの辺を使ったかを区別できるよう、ペイロードには元の入力での辺番号
    /// などを持たせておくとよい。存在しない場合は `None`。辺を1本も持たない
    /// グラフに対しては `Some((vec![], vec![]))` を返す。
    ///
    /// # Complexity
    /// - 時間計算量: O(V + E)
    ///   - V は頂点数、E は辺数である。
