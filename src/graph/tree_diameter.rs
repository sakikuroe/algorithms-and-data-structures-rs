...
impl<T> graph::Graph<T> {
    ...
    /// (辺数) であり、木でなければ、その理由を表すエラーである。頂点数が0
    /// の場合は `Ok(0)` を返す。
    ///
    /// # Complexity
    /// - 時間計算量: O(V)
    ...
    /// であり、木でなければその理由を表すエラーである。頂点数が0の場合は
    /// `Ok((0, Vec::new()))` を返す。
    ///
    /// # Complexity
    /// - 時間計算量: O(V)
...
impl<T> graph::Graph<T> {
    ...
    /// (最短路の重みの和) であり、木でなければその理由を表すエラーである。
    /// 頂点数が0の場合は `Ok(W::default())` を返す。
    ///
    /// # Constraints
...
impl<T: Copy + Ord + std::ops::Add<Output = T> + Default> graph::Graph<T> {
    ...
    /// 直径を実現する2頂点間の経路 (端点を含む頂点列) であり、木でなければ
    /// その理由を表すエラーである。頂点数が0の場合は
    /// `Ok((T::default(), Vec::new()))` を返す。
    ///
    /// # Constraints
    /// - すべての辺の重みが非負でなければならない。
    /// - `T::default()` が加法の単位元 (0 に相当する値) でなければならない。
