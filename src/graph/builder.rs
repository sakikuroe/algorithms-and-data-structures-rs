//! 同じ値かどうかを比較・判定できる任意の頂点ラベルから `Graph<T>` を構築する
//! ための補助役を提供するモジュールである。この補助役 (`GraphBuilder`) は、
//! 辺を1本ずつ受け取って蓄えておき、最後に `build` を呼んだ時点でまとめて
//! `Graph<T>` を組み立てる、という役割から「ビルダー」と呼ぶ。
//!
...
/// 同じ値かどうかを比較・判定できる任意の頂点ラベル `V` に頂点番号を自動採番
/// しながら、辺を1本ずつ蓄えておき、[`build`](Self::build) を呼んだ時点で
/// まとめて `Graph<T>` を組み立てる補助役である。
