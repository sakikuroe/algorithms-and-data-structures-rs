...
#[cfg(test)]
mod tests {
    /// Background: x + x^2 を表す、非ゼロ項が少ない疎な形式的べき級数。
    fn create_sparse_fps() -> super::super::FPS {
        super::super::FPS::new(vec![0, 1, 1])
    }

    /// Background: 非ゼロ項が多い密な形式的べき級数。
    fn create_dense_fps() -> super::super::FPS {
        super::super::FPS::new(vec![1, 2, 3, 4, 5, 6, 7, 8])
    }

    /// Background: 定数項が非ゼロの形式的べき級数。0 乗の境界値確認に用いる。
    fn create_fps_with_nonzero_constant() -> super::super::FPS {
        super::super::FPS::new(vec![2, 3, 4])
    }

    /// Background: 非ゼロ項が飛び飛びに現れる疎な形式的べき級数。
    fn create_another_sparse_fps() -> super::super::FPS {
        super::super::FPS::new(vec![0, 1, 0, 2, 0, 0, 3])
    }

    /// Background: 2x^2 + 3x^3 を表す、最小次数の項が 2 次にある形式的べき級数。
    fn create_fps_with_leading_term_at_degree_two() -> super::super::FPS {
        super::super::FPS::new(vec![0, 0, 2, 3])
    }

    /// Background: x^3 を表す、最小次数の項が 3 次にある形式的べき級数。
    fn create_fps_with_leading_term_at_degree_three() -> super::super::FPS {
        super::super::FPS::new(vec![0, 0, 0, 1])
    }

    // pow のテスト: 戻り値そのものを検証する
    mod pow {
        use super::*;

        /// Scenario: 指数が 0 のとき、元の級数によらず結果は定数項 1 の級数になる
        /// - Given: 定数項が非ゼロの形式的べき級数がある
        /// - When: 指数を 0 として pow を計算する
        /// - Then: 定数項が 1 になり、1 次以降の係数は 0 になる
        #[test]
        fn returns_constant_one_when_exponent_is_zero() {
            // Given
            let sut = create_fps_with_nonzero_constant();
            let degree = 5;

            // When
            let result = sut.pow(0, degree);
        ...
        /// Scenario: 疎な級数の 3 乗が既知の展開結果と一致する
        /// - Given: x + x^2 を表す疎な形式的べき級数がある
        /// - When: 3 乗を計算する
        /// - Then: (x + x^2)^3 = x^3 + 3x^4 + 3x^5 + x^6 と一致する
        #[test]
        fn matches_known_expansion_for_sparse_series() {
            // Given
            let sut = create_sparse_fps();
            let degree = 4;

            // When
            let result = sut.pow(3, degree);
    ...
    // pow_dense のテスト: 戻り値そのものを検証する
    mod pow_dense {
        use super::*;

        /// Scenario: 指数が 0 のとき、元の級数によらず結果は定数項 1 の級数になる
        /// - Given: 定数項が非ゼロの形式的べき級数がある
        /// - When: 指数を 0 として pow_dense を計算する
        /// - Then: 定数項が 1 になり、1 次以降の係数は 0 になる
        #[test]
        fn returns_constant_one_when_exponent_is_zero() {
            // Given
            ...
            // When
            let result = sut.pow_dense(0, degree);
        ...
        /// Scenario: 密な級数に対して、pow_dense は pow_sparse と同じ結果になる
        /// - Given: 非ゼロ項が多い密な形式的べき級数がある
        /// - When: pow_dense と pow_sparse をそれぞれ計算する
        /// - Then: 両者の結果は等しい
        #[test]
        fn matches_pow_sparse_for_dense_series() {
            // Given
            ...
            // When
            let dense = sut.pow_dense(exponent, degree);

            // Then
            let sparse = sut.pow_sparse(exponent, degree);
            assert_eq!(sparse, dense);
        }
    }

    // pow_sparse のテスト: 戻り値そのものを検証する
    mod pow_sparse {
        use super::*;
        ...
        /// - Then: (x + x^2)^3 = x^3 + 3x^4 + 3x^5 + x^6 と一致する
        #[test]
        fn matches_known_expansion_for_sample_problem() {
            // Given
            let sut = create_sparse_fps();
            let degree = 4;

            // When
            let result = sut.pow_sparse(3, degree);
        ...
        /// Scenario: 指数が 0 のとき、元の級数によらず結果は定数項 1 の級数になる
        /// - Given: 定数項が非ゼロの形式的べき級数がある
        /// - When: 指数を 0 として pow_sparse を計算する
        /// - Then: 定数項が 1 になり、1 次以降の係数は 0 になる
        #[test]
        fn returns_constant_one_when_exponent_is_zero() {
            // Given
            ...
            // When
            let result = sut.pow_sparse(0, degree);
        ...
        /// Scenario: 疎な級数に対して、pow_sparse は自動選択された pow と同じ結果になる
        /// - Given: 非ゼロ項が少ない疎な形式的べき級数がある
        /// - When: pow_sparse と pow をそれぞれ計算する
        /// - Then: 両者の結果は等しい
        #[test]
        fn matches_auto_selected_pow_for_sparse_series() {
            // Given
            let sut = create_another_sparse_fps();
            let exponent = 5;
            let degree = 12;
            ...
            // Then
            let auto = sut.pow(exponent, degree);
            assert_eq!(sparse, auto);
        }

        /// Scenario: 最小次数の項が複数あっても、シフトとスケーリングが正しく処理される
        /// - Given: 2x^2 + 3x^3 を表す形式的べき級数がある
        /// - When: 2 乗を計算する
        /// - Then: (2x^2 + 3x^3)^2 = 4x^4 + 12x^5 + 9x^6 と一致する
        #[test]
        fn handles_shift_and_scaling_for_leading_term() {
            // Given
            let sut = create_fps_with_leading_term_at_degree_two();
            let degree = 8;

            // When
            let result = sut.pow_sparse(2, degree);
        ...
        /// Scenario: 最小次数の項による移動量が要求次数を超える場合、結果はすべて 0 になる
        /// - Given: x^3 を表す形式的べき級数があり、指数 2 と移動量 (2 * 3 = 6) が
        ///   要求次数 4 を超える
        /// - When: 2 乗を計算する
        /// - Then: 0 次から degree 次までのすべての係数が 0 になる
        #[test]
        fn returns_zero_series_when_shift_exceeds_degree() {
            // Given
            let sut = create_fps_with_leading_term_at_degree_three();
            let degree = 4;
