...
/// 変換がサポートする最大長。
///
/// `998244353 - 1 = 119 * 2^23` であるため、法としては `2^23` 乗根まで存在する。
/// ただし `NTT_RATE`/`INTT_RATE` などの累積回転因子テーブルは、構築の過程で
/// 2 進の桁を 1 段消費する構造になっているため、このテーブルを使った NTT が
/// 実際に扱える最大長は `2^23` ではなく `2^22` である。`2^22` を超える値を
/// 設定すると、回転因子テーブルの範囲外アクセスでパニックする。
